...
export const getTodo = (req: Request<{ id: number }>, res: Response<Todo | ValidationError>) => {
...
export const addTodo = (req: Request<Record<string, any> | undefined, null, Todo>, res: Response<Todo | ValidationError>) => {
    const validationErrors = validationResult(req).array()
    if (validationErrors.length > 0) return res.status(400).send(validationErrors[0])
...
export const updateTodo = (req: Request<{ id: number }, null, Todo>, res: Response<Todo>) => {
    try {
        const id = Number(req.params.id)
